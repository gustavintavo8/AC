...
int main()
{
	...
	/* Calculate the frame and flags for the kernel code, data and bss pages:
		...
		dataFrame, dataFlags
		bssFrame,  bssFlags */
	/* Get the page table entry for the kernel code page */
 
 
	if (get_pte((void *)(kernelAddr.minKernelCode + OS_BASE_ADDR), &pte) < 0)
	{
		fprintf(stderr, "\nError. Try running with sudo\n\n");
		return EXIT_FAILURE;
	}
	...
	/* Get the page table entry for the kernel data page */
	if (get_pte((void *)(kernelAddr.minKernelData + OS_BASE_ADDR), &pte) < 0)
	{
		fprintf(stderr, "\nError. Try running with sudo\n\n");
		return EXIT_FAILURE;
	}
	...
	/* Get the page table entry for the kernel bss page */
	if (get_pte((void *)(kernelAddr.minKernelBss + OS_BASE_ADDR), &pte) < 0)
	{
		fprintf(stderr, "\nError. Try running with sudo\n\n");
		return EXIT_FAILURE;
	}
